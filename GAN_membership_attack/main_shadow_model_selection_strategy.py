...
for i in range(10):  # 10개의 Shadow 모델 생성
    shadow_model = create_model()
    shadow_model = train_model(shadow_model, shadow_data_train, shadow_labels_train)
    shadow_models.append(shadow_model)

    # 각 Shadow 모델의 정확도 계산
    shadow_accuracy = accuracy_score(shadow_labels_test, (predict_model(shadow_model, shadow_data_test) > 0.3).astype(int))
...
# 상위 30%로 우수한 Shadow 모델 선택
top_30_percent_index = np.argsort(shadow_model_accuracies)[-int(len(shadow_model_accuracies) * 0.3):]
best30_shadow_models = [shadow_models[i] for i in top_30_percent_index]

# 공격 모델 학습 데이터 준비 - 상위 30%로 우수한 Shadow 모델의 결과만 사용
best30_train_preds = np.concatenate([predict_model(model, shadow_data_train) for model in best30_shadow_models])
best30_test_preds = np.concatenate([predict_model(model, shadow_data_test) for model in best30_shadow_models])

x_attack_best30 = np.concatenate([best30_train_preds, best30_test_preds])
y_attack_best30 = np.concatenate([np.ones(len(best30_train_preds)), np.zeros(len(best30_test_preds))])
...
# 공격 모델 학습 - 상위 30%로 우수한 Shadow 모델의 결과만 사용
attack_model_best30 = create_model(input_shape=(1,))
attack_model_best30 = train_model(attack_model_best30, x_attack_best30, y_attack_best30)
...
# 멤버십 예측 수행 - 상위 30%로 우수한 Shadow 모델 결과만 사용
membership_predictions_best30 = predict_model(attack_model_best30, x_target_attack)
...
# 결과 출력 및 성능 지표 계산 - 상위 30%로 우수한 Shadow 모델 결과만 사용
print("\nResults using the Best30 Shadow Models:")
evaluate_membership_inference(y_target_attack, membership_predictions_best30)
